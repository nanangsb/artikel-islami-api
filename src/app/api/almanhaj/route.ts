...
export async function GET(request: NextRequest) {
  try {
    const ALMANHAJ_NEWS_RSS = "https://almanhaj.or.id/feed";
    const url = new URL(request.url);
    const searchParams = url.searchParams.get("search");
    const result = await parseRSS({
      url: ALMANHAJ_NEWS_RSS,
    });
    ...
    let responseData = {
      messages: "Result of all articles in Almanhaj.or.id",
      total: data.length,
      data,
    };
    ...
    if (searchParams) {
      ...
      responseData = {
        messages: `Result of all articles in Almanhaj.or.id with title search: ${searchParams}`,
