...
export async function GET(request: NextRequest) {
  try {
    const KHOTBAHJUMAT_NEWS_RSS = "https://khotbahjumat.com/feed";
    const url = new URL(request.url);
    const searchParams = url.searchParams.get("search");
    const result = await parseRSS({
      url: KHOTBAHJUMAT_NEWS_RSS,
    });
    ...
    let responseData = {
      messages: "Result of all articles in Khotbah Jumat",
      total: data.length,
      data,
    };
    ...
    if (searchParams) {
      ...
      responseData = {
        messages: `Result of all articles in Khotbah Jumat with title search: ${searchParams}`,
