...
export async function GET(request: NextRequest) {
  try {
    const RUMAYSHO_NEWS_RSS = "https://rumaysho.com/feed";
    const url = new URL(request.url);
    const searchParams = url.searchParams.get("search");
    const result = await parseRSS({
      url: RUMAYSHO_NEWS_RSS,
    });
    ...
    let responseData = {
      messages: "Result of all article in Rumaysho",
      total: data.length,
      data,
    };
    ...
    if (searchParams) {
      ...
      responseData = {
        messages: `Result of all article in Rumaysho with title search: ${searchParams}`,
